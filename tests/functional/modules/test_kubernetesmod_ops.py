...
pytestmark = [
    pytest.mark.skip_unless_on_linux(reason="kind cluster fixture requires Linux"),
    pytest.mark.skipif(
...
def test_exec_string_command_returns_stdout(kubernetes_exe, pod):
    ...
    res = kubernetes_exe.exec(name=pod["name"], command="echo hello-from-salt")
...
def test_exec_list_command_runs_directly(kubernetes_exe, pod):
    ...
    res = kubernetes_exe.exec(name=pod["name"], command=["cat", "/etc/hostname"])
    assert res["retcode"] == 0
...
def test_exec_nonzero_exit_propagates(kubernetes_exe, pod):
    ...
    res = kubernetes_exe.exec(name=pod["name"], command="exit 7")
    assert res["retcode"] == 7


def test_exec_stderr_captured(kubernetes_exe, pod):
    """stderr from the command is captured separately."""
    res = kubernetes_exe.exec(name=pod["name"], command="echo to-stderr 1>&2; exit 0")
    assert res["retcode"] == 0
...
def test_exec_stdin_fed_to_command(kubernetes_exe, pod):
    ...
    exec calls. The exec docstring spells this out.
    """
    payload = "payload-from-test\n"
    res = kubernetes_exe.exec(
        name=pod["name"],
...
def test_exec_stdin_eof_unsupported_command_times_out(kubernetes_exe, pod):
    ...
    """
    res = kubernetes_exe.exec(name=pod["name"], command=["cat"], stdin="x", timeout=3)
...
def test_exec_pod_not_found_raises(kubernetes_exe):
    ...
    with pytest.raises(CommandExecutionError, match="not found"):
        kubernetes_exe.exec(name="does-not-exist", command="true")
...
def test_logs_returns_string(kubernetes_exe, pod):
    ...
    res = kubernetes_exe.exec(name=pod["name"], command="sleep 1")
    assert res["retcode"] == 0
...
def test_cp_to_then_cp_from_round_trip(kubernetes_exe, pod, tmp_path):
    ...
    check = kubernetes_exe.exec(name=pod["name"], command=f"cat /tmp/{src.name}")
